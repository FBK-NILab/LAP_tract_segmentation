...
out_dir = sprintf('/N/dc2/projects/lifebid/giulia/data/HCP3_processed_data_trk/%s',sub);
...
if exist('/anat', 'file')
    ...
    %data
    t1_src=fullfile(strcat(src_dir,'/anat/',sub, '_t1.nii.gz'));
    fe_src=fullfile(strcat(sub,'_output_fe.mat'));
    fe_out=strcat(out_dir, '/', sub, '_output_fe.trk');
    afq_src=strcat(sub, '_output.mat');
    ...
    for tract=1:num_tracts
        afq_out=strcat(out_dir,'/',sub,'_',(sprintf('%s_tract.trk',strrep(fg_classified(tract).name,' ','_'))));
